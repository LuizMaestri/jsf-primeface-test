		...
		$('#rootwizard1').bootstrapWizard({
			...
			onTabClick: function (tab, navigation, index, clickedIndex) {
				return (clickedIndex < index) || 
					((clickedIndex > 0) && $('#toTab' + (clickedIndex - 1)).hasClass('valid')) || 
					$('#toTab' + clickedIndex).hasClass('valid');
			},
			onNext: function (tab, navigation, index, clickedIndex) {
				clickedIndex = index;
				index--;
				return (clickedIndex < index) || ((clickedIndex === index + 1) && tab.hasClass('valid')) || $('#toTab' + clickedIndex).hasClass('valid');
			}
		});
	};
